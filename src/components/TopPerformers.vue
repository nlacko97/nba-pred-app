...
const { allPlayerResults, userResults, loading, formattedDate } = storeToRefs(dailyResultsStore)
  ...
  <section v-if="!loading && topThree.length > 0" class="card mb-6">
    <div class="card-header flex justify-between items-center">
      <h2 class="text-lg font-bold flex items-center gap-2">
        ...
        Last Day's Winners
        <span class="text-xs font-normal text-gray-500 ml-2">({{ formattedDate }})</span>
      </h2>
    </div>
    ...
    <div class="p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
        <div 
          ...
          :class="[
            player.user_id === userId ? 'ring-2 ring-blue-500 ring-offset-2 dark:ring-offset-gray-900 border-blue-200 dark:border-blue-800' : 'border-gray-100 dark:border-gray-700'
          ]"
        >
          <!-- Rank -->
          <div class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-gray-50 dark:bg-gray-700/50 mr-3 font-mono font-bold text-gray-500 dark:text-gray-400">
            {{ index + 1 }}
          </div>
          ...
          <div class="flex-shrink-0 mr-3">
            ...
            >
              {{ (player.username || player.user_full_name || '?').charAt(0).toUpperCase() }}
            </div>
          </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mt-1">
              <span class="font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-1.5 py-0.5 rounded">
                {{ player.points }} Pts
              </span>
      ...
      <div v-if="userResults && !userInTopThree" class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
        <div class="flex items-center justify-between text-sm mb-2">
          <span class="text-gray-500 dark:text-gray-400 font-medium">Your Performance</span>
        </div>
        <div class="flex items-center p-4 rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50/30 dark:bg-blue-900/10">
          <div class="flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300 font-bold text-sm mr-3 font-mono">
          ...
          <div class="flex-shrink-0 mr-3">
            ...
            >
              {{ (userResults.username || userResults.user_full_name || 'Y').charAt(0).toUpperCase() }}
            </div>
          </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3 text-xs font-medium mt-1">
              <span class="text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/50 px-1.5 py-0.5 rounded">{{ userResults.points || 0 }} Pts</span>
